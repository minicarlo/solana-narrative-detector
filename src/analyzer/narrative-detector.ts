import { Narrative, NarrativeData, ProjectIdea } from '../types.js';
...
export class NarrativeDetector {
    private heliusCollector: HeliusCollector;
    private githubCollector: GitHubCollector;
    private socialCollector: SocialCollector;

    constructor() {
        this.heliusCollector = new HeliusCollector();
        this.githubCollector = new GitHubCollector();
        this.socialCollector = new SocialCollector();
    }

    async detectNarratives(): Promise<NarrativeData> {
    ...
    private extractHeliusSignals(data: any): RawSignal[] {
        const signals: RawSignal[] = [];
    ...
    private extractGitHubSignals(data: any): RawSignal[] {
        const signals: RawSignal[] = [];
    ...
    private extractSocialSignals(data: any[]): RawSignal[] {
        const signals: RawSignal[] = [];
    ...
    private analyzeSignals(signals: RawSignal[]): Narrative[] {
        const narratives: Narrative[] = [];

        for (const definition of NARRATIVE_DEFINITIONS) {
            // Find all signals matching this narrative
            const matchingSignals = signals.filter(signal => 
                signal.keywords.some(kw => 
                    definition.keywords.includes(kw.toLowerCase())
                )
            );
    ...
    private createEmptyNarrative(definition: any): Narrative {
        return {
            id: definition.id,
            name: definition.name,
            description: definition.description,
    ...
    private generateProjectIdeas(definition: any, keywords: string[]): string[] {
